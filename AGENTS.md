...
| `<phase-folder>/<netid>/` | students | one student's work: `draft.md`, `thinking/`, `review/`, `sessions/` |
...
Each student works in `<phase-folder>/<netid>/`. If the NetID isn't clear from the working directory or the conversation, ask for it once and then use that folder. Don't write into a teammate's folder.
